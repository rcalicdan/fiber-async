...
function run_async_performance_test(array $dbConfig, int $count, float $latency, int $poolSize): array
{
    ...
    return run(function () use ($count, $latency) {
        $startTime = microtime(true);
        $startMemory = memory_get_usage();
...
try {
    ...
    echo "  ($queryCount queries, " . ($latencySeconds * 1000) . "ms simulated latency each, Pool Size: $poolSize)\n";
    echo "=================================================\n";
    ...
    setup_performance_db($setupPdo, $queryCount);
    echo "[SETUP] Database is ready.\n";
    unset($setupPdo); // Close setup connection

    // --- Run Synchronous Test ---
    echo "\n-- [SYNC] Running performance test... --\n";
    $syncResult = run_sync_performance_test($mysqlConfig, $queryCount, $latencySeconds);
    echo "[SYNC] Test complete.\n";

    // --- Run Fiber-Async Test ---
    echo "\n-- [ASYNC Fiber-Async] Running performance test... --\n";
    $asyncFiberResult = run_async_performance_test($mysqlConfig, $queryCount, $latencySeconds, $poolSize);
    ...
    printf("| Sync (PDO)            | %-19s | %-22s | %-10s |\n", number_format($syncTime, 4) . ' s', number_format($syncMem / 1024, 2) . ' KB', number_format($queryCount / $syncTime, 2));

    $fiberTime = $asyncFiberResult['metrics']['time'];
    $fiberMem = $asyncFiberResult['metrics']['memory'];
    printf("| Async (Fiber-Async)   | %-19s | %-22s | %-10s |\n", number_format($fiberTime, 4) . ' s', number_format($fiberMem / 1024, 2) . ' KB', number_format($queryCount / $fiberTime, 2));
