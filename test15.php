...
function testCookieManagement() {
    ...
    try {
        ...
        $response4a = await($http->request()
            ->withPersistentCookies($cookieFile)
            ->get('https://httpbin.org/cookies/set/persistent/saved_value'));
        ...
        if ($response4b->successful()) {
            ...
            if (isset($data['cookies']['persistent'])) {
                printSuccess("Persistent cookie loaded from file: " . $data['cookies']['persistent']);
            } else {
        ...
        $request8 = $http->request()->cookieJarWith($attrJar);
