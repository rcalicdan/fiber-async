...
class AsyncOperations
{
     ...
     */
    public function runInBackground(callable $blockingOperation, array $args = []): PromiseInterface
    {
        return new AsyncPromise(function ($resolve, $reject) use ($blockingOperation, $args) {
            $processId = AsyncEventLoop::getInstance()->runInBackground($blockingOperation, $args);

            // Create a fiber to poll for completion
            $fiber = new Fiber(function () use ($processId, $resolve, $reject) {
                while (!AsyncEventLoop::getInstance()->isBackgroundTaskCompleted($processId)) {
                    Fiber::suspend();
                }

                try {
                    $result = AsyncEventLoop::getInstance()->getBackgroundResult($processId);
                    $resolve($result);
                } catch (Throwable $e) {
                    $reject($e);
                }
     ...
     */
    public function runConcurrentlyInBackground(array $tasks, int $maxConcurrency = 4): PromiseInterface
    {
        return new AsyncPromise(function ($resolve, $reject) use ($tasks, $maxConcurrency) {
            if (empty($tasks)) {
                $resolve([]);
                return;
            }
            ...
            $processNext = function () use (&$processNext, &$tasks, &$running, &$completed, &$results, &$total, &$taskIndex, $maxConcurrency, $resolve, $reject) {
                while ($running < $maxConcurrency && $taskIndex < $total) {
                    $currentIndex = $taskIndex++;
                    $task = $tasks[$currentIndex];
                    $running++;

                    $this->runInBackground($task['callable'], $task['args'] ?? [])
                        ->then(function ($result) use ($currentIndex, &$results, &$running, &$completed, $total, $resolve, $processNext) {
                            $results[$currentIndex] = $result;
                            $running--;
                            $completed++;
